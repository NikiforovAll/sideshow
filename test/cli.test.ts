...
for (const cmd of ["serve", "publish", "update", "wait", "watch", "comment", "list"]) {
...
test("watch streams each new user comment as one line and re-arms", async () => {
  ...
  try {
    ...
    // first comment, on a snippet — should surface with its title and id
    ...
    assert.match(stdout, /sideshow comment on “Doc” \(snippet .+\): “tighten the spacing”/);
